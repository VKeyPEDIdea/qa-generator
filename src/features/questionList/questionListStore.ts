...
class QuestionListStore implements IQuestionListStore {
  ...
  deleteAnswerByQuestionId = (id: string, answerText?: string) => {
    const filterByIdAndText = ({ text, questionId }: Answer) =>
      questionId !== id || text !== answerText;
    const filterById = ({ questionId }: Answer) => questionId !== id;
    const updatedAnswerList = this.answerList.filter(
      answerText ? filterByIdAndText : filterById,
    );
  ...
  deleteQuestion = (id: string) => {
    const updatedQuestionList = this.questionList.filter(
      (item) => item.id !== id,
    );
  ...
  getRandomAnswer(answerList: Answer[]): Answer | null {
    ...
    const reducedList = answerList.filter(
      (answer) => answer.text !== selected.text,
    );
  ...
  changeAnswerPercentage = (
    ...
    const targetAnswer = this.answerList.find(
      ({ text, questionId }) => questionId === id && text === answerText,
    );
    if (targetAnswer) {
  ...
  saveProject() {
    saveToStorage(this.projectTitle, {
